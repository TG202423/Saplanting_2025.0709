...
class MasterSetting(ServerSavableConfig):
    """
    服务器端的主配置类，继承自ServerSavableConfig，
    负责落地生根模块的各种配置管理，
    包括种子白名单、连锁砍树开关及参数等。
    """
    ...
    def load_data(self, data):
        """
        载入数据时的预处理：
        - min_wait_time 不允许小于0
        - saplings 转换为 set(tuple)
        - log_blocks 转换为 set
        调用父类完成剩余数据加载。
        """
    ...
    def dump(self):
        """
        保存数据时的预处理：
        - saplings 从 set(tuple) 转成 list[list] 方便序列化
        - log_blocks 从 set 转成 list
        返回处理后的数据字典。
        """
    ...
    def get_client_data(self, add_min_wait_time=True, add_saplings=True):
        """
        生成发送给客户端的配置数据：
        - add_min_wait_time: 是否添加最小等待时间字段
        - add_saplings: 是否添加种子白名单字段
        返回包含对应配置的字典。
        """
        data = {}
